...
export default function Information(props) {
  ...
  return (
      <main className='flex-1  p-4 flex flex-col gap-3 text-center sm:gap-4 w-72  justify-center pb-20 w-fit max-w-prose w-full mx-auto'>
       <h1 className='font-semibold text-5xl sm:text-6xl md:text-7xl whitespace-wrap'>Your <span className='text-purple-400 bold'>Transcription</span></h1>
       <div className='grid grid-cols-2 mx-auto bg-white shadow rounded-full overflow-hidden items-center'>
            <button onClick={() => setTab('transcription')} className={'px-4 duration-200 py-1 font-medium' + (tab === 'transcription' ? ' bg-purple-400 text-white' : ' text-purple-400 hover:text-purple-600')}>Transcription</button>
            <button onClick={() => setTab('translation')} className={'px-4 duration-200 py-1 font-medium' + (tab === 'translation' ? ' bg-purple-400 text-white' : ' text-purple-400 hover:text-purple-600')}>Translation</button>
       </div>
